function [no,xo] = histw(wt,varargin)
%HISTW  Weighted Histogram.
%   N = HIST(W,Y) bins the elements of Y into 10 equally spaced containers
%   and returns the number of elements in each container.  If Y is a
%   matrix, HIST works down the columns.
%
%   N = HIST(W,Y,M), where M is a scalar, uses M bins.
%
%   N = HIST(W,Y,X), where X is a vector, returns the distribution of Y
...
%
%   [N,X] = HIST(W,...) also returns the position of the bin centers in X.
%
...
if any(isnan(y.*wt))
    fprintf(1,'remove %d NaN from %d events\n',sum(isnan(y.*wt)),length(y))
    k=find(~isnan(y.*wt));
    y=y(k);
    wt=wt(k);
end
...
if isempty(y),
    ...
    %=====
    nn=0*nn;
    nn(1:max(BIN)) = accumarray(BIN, wt);
    %
    %for k=1:length(nn)
    %    nn(k)=sum(wt(BIN==k));
    %end
